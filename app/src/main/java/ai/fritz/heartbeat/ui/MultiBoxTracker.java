...
import ai.fritz.heartbeat.env.BorderedText;
import ai.fritz.heartbeat.env.ImageUtils;
...
public class MultiBoxTracker {
    ...
    public synchronized void trackResults(
            final List<FritzVisionObject> results, final byte[] frame, final long timestamp) {
    ...
    private void processResults(
            final long timestamp, final List<FritzVisionObject> results, final byte[] originalFrame) {
        final List<Pair<Float, FritzVisionObject>> rectsToTrack = new LinkedList<Pair<Float, FritzVisionObject>>();

        screenRects.clear();
        final Matrix rgbFrameToScreen = new Matrix(getFrameToCanvasMatrix());

        for (final FritzVisionObject result : results) {
            if (result.getBoundingBox() == null) {
                continue;
            }
            final RectF detectionFrameRect = new RectF(result.getBoundingBox());

            final RectF detectionScreenRect = new RectF();
            rgbFrameToScreen.mapRect(detectionScreenRect, detectionFrameRect);


            screenRects.add(new Pair<Float, RectF>(result.getVisionLabel().getConfidence(), detectionScreenRect));

            if (detectionFrameRect.width() < MIN_SIZE || detectionFrameRect.height() < MIN_SIZE) {
                continue;
            }

            rectsToTrack.add(new Pair<Float, FritzVisionObject>(result.getVisionLabel().getConfidence(), result));
        }
        ...
        if (objectTracker == null) {
            ...
            for (final Pair<Float, FritzVisionObject> potential : rectsToTrack) {
                final TrackedRecognition trackedRecognition = new TrackedRecognition();
                trackedRecognition.detectionConfidence = potential.first;
                trackedRecognition.location = new RectF(potential.second.getBoundingBox());
                trackedRecognition.trackedObject = null;
                trackedRecognition.title = potential.second.getVisionLabel().getText();
    ...
    private void handleDetection(
            final byte[] frameCopy, final long timestamp, final Pair<Float, FritzVisionObject> potential) {
        final ObjectTracker.TrackedObject potentialObject =
                objectTracker.trackObject(potential.second.getBoundingBox(), timestamp, frameCopy);
